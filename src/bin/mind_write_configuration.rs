...
use mind::{ConfigurationError, MindDaemonConfiguration, MindKnowledgeJudgeSocketConfiguration};
...
enum ConfigurationWriterKnowledgeJudge {
    ...
    MindJudge(ConfigurationWriterMindJudge),
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct ConfigurationWriterMindJudge {
    socket_path: ConfigurationWriterPath,
...
impl ConfigurationWriteRequest {
    ...
    fn configuration(self) -> Result<MindDaemonConfiguration, ConfigurationWriterError> {
        ...
        let configuration = match self.knowledge_judge {
            ...
            ConfigurationWriterKnowledgeJudge::MindJudge(knowledge_judge) => {
                configuration.with_mind_judge(knowledge_judge.into_runtime()?)
            }
        };
...
impl ConfigurationWriterKnowledgeJudge {
    fn from_nota_block(block: &nota::Block) -> Result<Self, NotaDecodeError> {
        ...
        match objects[0].demote_to_string() {
            ...
            Some("MindJudge") if objects.len() == 3 => {
                Ok(Self::MindJudge(ConfigurationWriterMindJudge {
                    socket_path: ConfigurationWriterPath::from_nota_block(&objects[1])?,
...
impl ConfigurationWriterMindJudge {
    fn into_runtime(
        self,
    ) -> Result<MindKnowledgeJudgeSocketConfiguration, ConfigurationWriterError> {
        Ok(MindKnowledgeJudgeSocketConfiguration::new(
            self.socket_path.into_wire_path()?,
